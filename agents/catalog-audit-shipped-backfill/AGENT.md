# Catalog-Audit Shipped Backfill — Runbook
...
- Triggered by:
  ...
  - Manual: `curl -X POST http://localhost:8090/api/agents/catalog-audit-shipped-backfill/trigger`
  - UI: http://localhost:8091/agents/catalog-audit-shipped-backfill → "▶ Run now"
...
- Always: `agents/catalog-audit-shipped-backfill/runs/<run-ts>/{progress,errors,decisions,context-summary}.{json,jsonl,md}` in the framework storage backend.
...
- `agents/catalog-audit-shipped-backfill/state/latest.json` — the most recent state object
- `agents/catalog-audit-shipped-backfill/state/history/<run-ts>.json` — historical snapshots
...
Goals live in `agents/catalog-audit-shipped-backfill/goals/current.json` and get scored
...
- _(Logs are at `/tmp/reusable-agents-logs/catalog-audit-shipped-backfill-*.log` if invoked
