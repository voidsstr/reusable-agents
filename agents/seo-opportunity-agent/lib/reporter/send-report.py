...
def _write_auto_queue(cfg, run_dir: Path, run_ts: Optional[str] = None) -> None:
    """Direct-dispatch every rec to the implementer via framework.core.dispatch.
    Same dispatch path as a real "implement all" email reply, no IMAP
    round-trip needed. Site-level lock serializes per-site, retries on
    transient failures, falls back to writing agents/responder-agent/auto-queue/
    if all retries fail (transitional). Emails operator on permanent failure.
    """
    try:
        ...
        try:
            handle = dispatch.dispatch_now(
                ...
                site=cfg.site_id, subject_tag="seo",
                request_id=request_id,
            )
            label = "via auto-queue (fallback)" if handle.fell_back_to_queue else "via direct dispatch"
            print(f"[reporter] auto-queued {len(rec_ids)} recs for implementation {label} "
                  f"(request_id={request_id})", file=sys.stderr)
        except Exception as e:
            print(f"[reporter] direct dispatch failed: {e}", file=sys.stderr)
    except Exception as e:
        print(f"[reporter] could not enqueue: {e}", file=sys.stderr)
