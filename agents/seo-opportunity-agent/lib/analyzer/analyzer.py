...
def _find_repo_root() -> Path:
    """Walk up to the checkout root (the dir holding framework/ + shared/).

    This file exists at TWO depths — agents/seo-analyzer/analyzer.py and
    agents/seo-opportunity-agent/lib/analyzer/analyzer.py — and the two
    copies are kept byte-identical, so a fixed `.parent.parent.parent`
    cannot be right for both. It resolved correctly for the standalone
    copy and silently pointed at agents/seo-opportunity-agent/ for the
    production one, which is why `_crawl_for_audit` died with
    "No module named 'crawler'": it built
    <root>/agents/progressive-improvement-agent from a bogus root.
    PYTHONPATH masked the breakage for `framework.*` / `shared.*`
    imports, so only the explicitly-constructed path failed.
    """
    here = Path(__file__).resolve()
    for cand in here.parents:
        if (cand / "framework").is_dir() and (cand / "shared").is_dir():
            return cand
    # Fall back to the historical guess rather than raising at import.
...
def _add_article_template_recs(data: Path, recs: list, next_id, max_recs: int) -> None:
    """Flag review pages missing 5+ of the 9 canonical sections
    (methodology, specs, independent-testing, owner-feedback, strengths,
    limitations, buyer-persona, alternatives, citations).

    The 9-section template is review-shaped (hardware, software,
    product) — it does NOT fit non-review editorial like meal plans,
    diet guides, ingredient features, seasonal occasion content. Only
    fire on `type=review` so AislePrompt's blog buckets (and any other
    site's editorial) don't get false-positive recs flagging them as
    "incomplete reviews". The lighter `faq-quality-thin` and
    `eeat-outbound-citation-count` rules still cover blog-type pages.
    """
    if len(recs) >= max_recs:
        return
    pages = _load_pages_by_type(data)
