...
| `digest-rollup-agent` | `agents/digest-rollup-agent/` | `0 */3 * * *` | Drains `digest-queue/` (every per-agent email gets queued there when `DIGEST_ONLY=1`), groups by site + agent, sends ONE summary email. **Header now includes per-site SEO + Amazon-revenue readiness metrics** pulled live from the DB each render: editorial coverage % (ASINs with ≥1 editorial inbound link), featured-product price freshness (≤24h target), stale-priced Amazon catalog count, published content totals (articles / buying-guides / head-to-head pairs / researched-topic queue depth). Pulls per-site DSN from `DATABASE_URL_<UPPER_SITE>` env. |
...
|---|---|---|---|---|
| `specpicks-product-hydration-agent` | reusable: `agents/product-hydration-agent/` | `specpicks/agents/product-hydration-agent/` | `15 */2 * * *` | Every 2h: refreshes Amazon-sourced product pricing via BrightData (default — falls back to PA-API once SpecPicks qualifies for Associates). **Cap 500/run = 6,000/day**. Selection is **tiered** when `prioritize_featured_and_linked: true` (tier 0 = is_featured, tier 1 = ASINs referenced in editorial_articles or buying_guides, tier 2 = high-review-count tail) — this keeps the user-visible product set <24h fresh while the long tail catches up over ~12 days. Then Claude-driven content hydration. Final phase recomputes the `is_featured` set via `select-featured.py`. |
